...
users_db = {}
# ИЗМЕНЕНО: Теперь characters_db хранит ВСЕХ персонажей глобально
# Структура: {character_id: {данные персонажа + creator_id}}
characters_db = {}
chats_db = {}  # {user_id: {character_id: [messages]}}
profiles_db = {}

user_id_counter = 1
character_id_counter = 1  # ДОБАВЛЕНО: счетчик для ID персонажей
...
# ИЗМЕНЕНО: Теперь возвращает ВСЕХ персонажей с информацией о создателе
...
def get_characters(current_user_id):
    try:
        ...
        for char_id, char_data in characters_db.items():
            ...
            # Добавляем флаг, является ли текущий пользователь создателем
        ...
        # Сортируем: сначала свои персонажи, потом чужие
...
# ИЗМЕНЕНО: Создание персонажа теперь добавляет creator_id
...
def create_character(current_user_id):
    try:
        ...
        character = {
            ...
            'greeting': greeting,
            'avatar': avatar,
            'creator_id': current_user_id,  # ДОБАВЛЕНО
            'created_at': datetime.datetime.utcnow().isoformat()
        }
...
# ИЗМЕНЕНО: Удаление одного персонажа (только создатель)
...
def delete_character(current_user_id, character_id):
    try:
        ...
        # Удаляем все чаты с этим персонажем
...
# ИЗМЕНЕНО: Удаление всех СВОИХ персонажей
...
def delete_all_characters(current_user_id):
    try:
        ...
        # Находим всех персонажей пользователя
        for char_id, char_data in characters_db.items():
...
# ИЗМЕНЕНО: Список чатов теперь использует character_id
...
# ИЗМЕНЕНО: Используем character_id вместо position
...
# ИЗМЕНЕНО: Используем character_id
