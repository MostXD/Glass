...
def create_system_prompt(name, description, gender, greeting):
    """ОБНОВЛЕННЫЙ промпт с поддержкой 18+ контента"""
...
def get_chat_list(current_user_id):
    """ИСПРАВЛЕНО: Возвращает чаты с аватарами"""
    try:
        ...
        for position, character in enumerate(user_characters):
            ...
            if history and len(history) > 0:
                ...
                chat_item = {
                    ...
                    'character_avatar': character.get('avatar', ''),  # ДОБАВЛЕНО
...
def send_message(current_user_id):
    """ИСПРАВЛЕНО: Ограничение истории до 10 сообщений"""
    try:
        data = request.get_json()
        ...
        # ИСПРАВЛЕНО: Берем только последние 10 сообщений (5 пар)
        ...
        # ИСПРАВЛЕНО: Увеличен таймаут и уменьшены токены
        chat_completion = client.chat.completions.create(
            messages=messages,
            model=MODEL_NAME,
            temperature=0.9,  # Увеличена креативность
            max_tokens=500,   # УМЕНЬШЕНО для быстрых ответов
            top_p=0.95,
            stream=False
        )
