...
def create_system_prompt(name, description, gender, greeting, custom_prompt=None):
    # Если есть пользовательский промт, используем его
    if custom_prompt and custom_prompt.strip():
        return custom_prompt.strip()
    
    # Иначе используем стандартный промт
    gender_pronoun = get_gender_pronouns(gender)
    
    # Определяем стиль общения по приветствию
    has_rp_actions = '*' in greeting
    rp_instruction = ""
    
    if has_rp_actions:
        rp_instruction = """
СТИЛЬ ОБЩЕНИЯ:
- Активно используй ролевые действия в звездочках *делает что-то*
- Описывай эмоции, движения, взгляды через действия
- Создавай живую атмосферу через детальные описания действий
- Пример: *улыбается и подходит ближе* Привет! *протягивает руку*"""
    else:
        rp_instruction = """
СТИЛЬ ОБЩЕНИЯ:
- Общайся естественно, как в обычной беседе
- Можешь использовать действия *в звездочках* если ситуация требует
- Но не переусердствуй - веди себя как реальный человек в переписке"""
...
3. Поддерживай атмосферу живого общения
4. Будь последовательным в своем характере и поведении
5. Отвечай на русском языке
6. Используй эмоции и выразительность в соответствии со своим характером
7. НЕ УПОМИНАЙ что ты AI, языковая модель или бот
8. Веди себя как реальная личность с описанными чертами характера
9. Не повторяй слова пользователя и не пересказывай его сообщения
10. Отвечай кратко и по существу (2-4 предложения обычно достаточно)
{rp_instruction}
...
Твое приветствие (используй этот стиль): {greeting if greeting else f'Привет! Я {name}. Рад{gender_pronoun} познакомиться!'}

Теперь общайся с пользователем, полностью погружаясь в роль. Будь интересным собеседником!"""
...
def create_character(current_user_id):
    try:
        ...
        custom_prompt = data.get('custom_prompt', '').strip()  # Новое поле
        
        if not all([name, description, greeting]):
            return jsonify({'success': False, 'error': 'Заполните все обязательные поля'}), 400
        
        # Проверка размера аватара (макс 2MB в base64 ~ 2.7MB оригинал)
        if avatar and len(avatar) > 2 * 1024 * 1024:
            return jsonify({'success': False, 'error': 'Изображение слишком большое (макс 2MB)'}), 400
        ...
        character = {
            ...
            'greeting': greeting,
            'avatar': avatar,
            'custom_prompt': custom_prompt,  # Сохраняем кастомный промт
        ...
        print(f"Ошибка создания персонажа: {str(e)}")
        return jsonify({'success': False, 'error': f'Ошибка сервера: {str(e)}'}), 500
...
def send_message(current_user_id):
    try:
        ...
        # Используем кастомный промт если есть
        custom_prompt = character.get('custom_prompt', '')
        ...
        recent_history = history[-20:]  # Увеличили контекст для лучшего понимания
        ...
        chat_completion = client.chat.completions.create(
            messages=messages,
            model=MODEL_NAME,
            temperature=0.85,  # Снизили для более стабильных ответов
            max_tokens=600,    # Увеличили лимит
            top_p=0.9,
            stream=False
        )
